...
export const libraries = pgTable(
  ...
  {
    id: serial('id').primaryKey(),
    libCode: varchar('lib_code', { length: 16 }).notNull().unique(),
    name: varchar('name', { length: 128 }).notNull(),
    address: varchar('address', { length: 256 }).notNull(),
    region: varchar('region', { length: 64 }).notNull(),
    detailRegion: varchar('detail_region', { length: 64 }),
    lat: doublePrecision('lat').notNull(),
    lng: doublePrecision('lng').notNull(),
    location: geography('location'),
    phone: varchar('phone', { length: 32 }),
    homepage: varchar('homepage', { length: 256 }),
    closedDays: varchar('closed_days', { length: 256 }),
    operatingHours: jsonb('operating_hours'),
    type: varchar('type', { length: 32 }),
    updatedAt: timestamp('updated_at').notNull().defaultNow(),
  },
  (t) => ({
    libCodeIdx: uniqueIndex('libraries_lib_code_idx').on(t.libCode),
    regionIdx: index('libraries_region_idx').on(t.region),
    locationIdx: index('libraries_location_gix').on(t.location).using(sql`gist`),
  }),
)

export const libraryCards = pgTable(
  'library_cards',
  {
    id: serial('id').primaryKey(),
...
export const bookCache = pgTable(
  ...
  (t) => ({
    ...
    titleTrgmIdx: index('book_cache_title_trgm_idx').on(t.title).using(sql`gin (title gin_trgm_ops)`),
    authorTrgmIdx: index('book_cache_author_trgm_idx').on(t.author).using(sql`gin (author gin_trgm_ops)`),
...
export const popularBooks = pgTable(
  'popular_books',
  {
    id: serial('id').primaryKey(),
    region: varchar('region', { length: 64 }).notNull(),
